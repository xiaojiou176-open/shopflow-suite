...
- `pnpm capture:ui-surfaces` now reuses the same headless extension harness to emit PNG previews for:
  - store popup
  - store sidepanel
  - Suite sidepanel
...
- current core extension surfaces keep one main landmark, no horizontal overflow,
  and hash-routed disclosures auto-open when the route contract says they should
...
- PNG previews are screenshot artifacts for repo-owned UI review, not live merchant proof
