...
This directory is the canonical in-repo packet rack for smaller packet-only
handoff surfaces.
...
It gives L1 one smaller packet-only rack without pretending it is the main
Shopflow repo.
...
1. Keep this directory inside the canonical repo as the smaller packet-only
   rack when a target-specific handoff still helps.
